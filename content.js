(() => {
  ...
  let records = [];
  let prefs = { section: "dashboard", view: "revenue", range: "all", interval: "auto", start: "", end: "", calendarMetric: "sales", lifetimeMetric: "revenue", lifetimeStyle: "area", lifetimeAlign: "calendar", lifetimeStackDefaultApplied: true, lifetimePackages: [], lifetimeHiddenPackages: [], sankeyPackages: [], sankeyGroupBy: "category", sankeyCategoryDefaultApplied: true };
  let syncJob = null;
  ...
  function revenueMixViewModel(items) {
    const packages = new Map();
    for (const item of items) {
      ...
      current.value += toNumber(item.gross); packages.set(key, current);
    }
    ...
    const visible = ranked.length > 6
      ? [...ranked.slice(0, 5), { key: "other", name: "Other packages", value: ranked.slice(5).reduce((sum, item) => sum + item.value, 0) }]
      : ranked;
  ...
  function renderRevenueMixChart(viewModel) {
    ...
    chart.setOption({
      animation: false,
      aria: { enabled: true, description: `Lifetime gross revenue split across ${viewModel.packageCount} revenue-generating packages.` },
      tooltip: {
        trigger: "item", confine: true, backgroundColor: "#151927", borderWidth: 0, padding: [10, 12], textStyle: { color: "#fff", fontSize: 11 },
        formatter: parameter => `<strong>${escapeHtml(parameter.name)}</strong><br/><span style="color:#c7c2e8">Gross revenue</span>&nbsp;&nbsp;${fullMoney(parameter.value)}<br/><span style="color:#c7c2e8">Portfolio share</span>&nbsp;&nbsp;${percent(parameter.percent)}`
      },
      series: [{
        name: "Lifetime revenue mix", type: "pie", radius: ["65%", "86%"], center: ["50%", "50%"], avoidLabelOverlap: true, selectedMode: false,
  ...
  function render() {
    ...
    const revenueMixData = revenueMixViewModel(records.filter(item => item.type === "sales"));
