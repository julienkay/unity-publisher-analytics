(() => {
  ...
  let records = [];
  let prefs = { section: "dashboard", view: "revenue", range: "all", interval: "auto", start: "", end: "", calendarMetric: "sales", lifetimeMetric: "revenue", lifetimeStyle: "lines", lifetimeAlign: "calendar", lifetimePackages: [], lifetimeHiddenPackages: [], sankeyPackages: [], sankeyGroupBy: "category", sankeyCategoryDefaultApplied: true };
  let syncJob = null;
  ...
  const LIFETIME_METRICS = {
    revenue: { id: "revenue", label: "Gross revenue", noun: "revenue", summaryLabel: "Revenue shown", rankingLabel: "revenue", field: "gross", source: "sales", currency: true, ageLabel: "Since first sale", ageDescription: "first sale", emptyLabel: "No package revenue is available yet." },
    sales: { id: "sales", label: "Sales quantity", noun: "sales", summaryLabel: "Sales shown", rankingLabel: "sales", field: "qty", source: "sales", currency: false, ageLabel: "Since first sale", ageDescription: "first sale", emptyLabel: "No package sales are available yet." },
    downloads: { id: "downloads", label: "Downloads", noun: "downloads", summaryLabel: "Downloads shown", rankingLabel: "downloads", field: "downloads", source: "downloads", currency: false, ageLabel: "Since first download", ageDescription: "first download", emptyLabel: "No package downloads are available yet." },
    pageviews: { id: "pageviews", label: "Pageviews", noun: "pageviews", summaryLabel: "Pageviews shown", rankingLabel: "pageviews", field: "pageViews", source: "daily", currency: false, ageLabel: "Since first pageview", ageDescription: "first pageview", emptyLabel: "No package pageviews are available yet." }
  };
  ...
  function lifetimeViewModel(items, requestedMetric, selectedPackageKeys, hiddenPackageKeys, requestedStyle, requestedAlign) {
    ...
    return { metric, options, explicitKeys, activePackages, legend, series, total, leader, pointCount, align, style };
  }
      ...
      <div class="upa-shell">
        ...
        <section class="upa-workspace">
          ...
          <main class="upa-content" data-section="${section}" data-view="${view}">${records.length ? `<section class="upa-kpis upa-view-panel upa-view-dashboard" id="upa-view-dashboard"><article><div><small>Gross revenue</small><span class="upa-kpi-dot upa-violet"></span></div><strong>${money(revenueChartData.total)}</strong><span>${number(paidUnits)} paid units in the selected period</span></article><article><div><small>Average monthly revenue</small><span class="upa-kpi-dot upa-violet"></span></div><strong>${money(trailingRevenue.monthlyAverage)}</strong><span>${trailingRevenue.monthCount === 12 ? "Last 12 complete months" : trailingRevenue.monthCount ? `Based on ${number(trailingRevenue.monthCount)} available months` : "No complete month available"}</span></article><article><div><small>Revenue growth</small><span class="upa-kpi-dot ${revenueGrowthDot}"></span></div><strong>${revenueGrowthValue}</strong><span>${trailingRevenue.hasBaseline ? "Last 12 months versus previous 12" : "Needs 24 months of history"}</span></article><article><div><small>Pageviews</small><span class="upa-kpi-dot upa-cyan"></span></div><strong>${number(pageViews)}</strong><span>${number(salesQty)} purchases and claims</span></article><article><div><small>Conversion rate</small><span class="upa-kpi-dot upa-green"></span></div><strong>${percent(conversionRate)}</strong><span>Views that became purchases or claims</span></article><article><div><small>Downloads</small><span class="upa-kpi-dot upa-amber"></span></div><strong>${number(downloads)}</strong><span>Across the selected period</span></article><article class="upa-dashboard-chart"><div class="upa-section-title"><div><small>PORTFOLIO PULSE</small><h2>Business activity over time</h2><p>${intervalName(overviewChartData.interval)} revenue, pageviews, and downloads on aligned timelines.</p></div><div class="upa-section-tools"><span>${overviewChartData.points.length} periods</span>${chartActions("overview")}</div></div><div class="upa-pulse-legend"><span><i class="upa-pulse-revenue"></i>Gross revenue</span><span><i class="upa-pulse-views"></i>Pageviews</span><span><i class="upa-pulse-downloads"></i>Downloads</span></div><div id="upa-overview-chart" class="upa-overview-chart" role="img" aria-label="Aligned gross revenue, pageviews, and downloads timelines"></div></article>${dashboardPackageTable}</section>
            ...
            <section class="upa-card upa-insight-card upa-view-panel upa-view-lifetime" id="upa-view-lifetime"><div class="upa-section-title"><div><small>LIFETIME GROWTH</small><h2>How packages accumulate ${escapeHtml(lifetimeData.metric.noun)}</h2><p>Cumulative ${escapeHtml(lifetimeData.metric.label.toLowerCase())} across all available history makes momentum and plateaus visible.</p></div><div class="upa-section-tools"><span>${lifetimeData.pointCount} months</span>${chartActions("lifetime", !lifetimeData.series.length)}</div></div><div class="upa-insight-toolbar upa-lifetime-toolbar"><div class="upa-lifetime-controls"><label class="upa-inline-select">Metric<select id="upa-lifetime-metric">${Object.values(LIFETIME_METRICS).map(metric => `<option value="${metric.id}" ${lifetimeData.metric.id === metric.id ? "selected" : ""}>${metric.label}</option>`).join("")}</select></label><label class="upa-inline-select">View<select id="upa-lifetime-style"><option value="lines" ${lifetimeData.style === "lines" ? "selected" : ""}>Cumulative lines</option><option value="area" ${lifetimeData.style === "area" ? "selected" : ""}>Stacked area</option></select></label>${lifetimeData.style === "lines" ? `<label class="upa-inline-select">Align<select id="upa-lifetime-align"><option value="calendar" ${lifetimeData.align === "calendar" ? "selected" : ""}>Calendar time</option><option value="age" ${lifetimeData.align === "age" ? "selected" : ""}>${lifetimeData.metric.ageLabel}</option></select></label>` : ""}<details class="upa-package-filter"><summary><span>Packages</span><strong>${lifetimeData.explicitKeys.length ? `${lifetimeData.explicitKeys.length} selected` : `Top 8 by ${lifetimeData.metric.rankingLabel}`}</strong></summary><div class="upa-package-filter-panel"><div class="upa-package-filter-head"><span>Choose packages to compare</span><button data-action="lifetime-top">Use top 8</button></div><div class="upa-package-checklist">${lifetimeData.options.map(item => `<label><input type="checkbox" data-lifetime-package="${escapeHtml(item.key)}" ${lifetimeData.activePackages.some(active => active.key === item.key) ? "checked" : ""}><span><strong>${escapeHtml(item.name)}</strong><small>${lifetimeValue(lifetimeData.metric, item.total)}</small></span></label>`).join("")}</div></div></details></div><div class="upa-insight-facts"><span><small>${lifetimeData.metric.summaryLabel}</small><strong>${lifetimeValue(lifetimeData.metric, lifetimeData.total)}</strong></span><span><small>Packages</small><strong>${number(lifetimeData.series.length)}</strong></span><span><small>Largest lifetime</small><strong>${lifetimeData.leader ? lifetimeValue(lifetimeData.metric, lifetimeData.leader.total) : "—"}</strong></span></div></div><div class="upa-lifetime-legend">${lifetimeData.legend.map(item => `<button type="button" data-lifetime-legend-package="${escapeHtml(item.key)}" aria-pressed="${item.visible}" title="${item.visible ? "Hide" : "Show"} ${escapeHtml(item.name)}"><i style="background:${item.color}"></i><strong>${escapeHtml(item.name)}</strong><em>${lifetimeValue(lifetimeData.metric, item.total)}</em></button>`).join("")}</div><div id="upa-lifetime-chart" class="upa-lifetime-chart" role="img" aria-label="Cumulative ${escapeHtml(lifetimeData.metric.label.toLowerCase())} by package"></div></section>
  ...
  async function init() {
    ...
    prefs = {
      ...
      calendarMetric: storedPrefs.calendarMetric || "sales", lifetimeMetric: LIFETIME_METRICS[storedPrefs.lifetimeMetric] ? storedPrefs.lifetimeMetric : "revenue", lifetimeStyle: storedPrefs.lifetimeStyle === "area" && storedLifetimeAlign === "calendar" ? "area" : "lines", lifetimeAlign: storedLifetimeAlign, lifetimePackages: Array.isArray(storedPrefs.lifetimePackages) ? storedPrefs.lifetimePackages : [], lifetimeHiddenPackages: Array.isArray(storedPrefs.lifetimeHiddenPackages) ? storedPrefs.lifetimeHiddenPackages : [], sankeyPackages: Array.isArray(storedPrefs.sankeyPackages) ? storedPrefs.sankeyPackages : [], sankeyGroupBy: storedPrefs.sankeyCategoryDefaultApplied === true ? storedSankeyGroupBy : "category", sankeyCategoryDefaultApplied: true
    };
