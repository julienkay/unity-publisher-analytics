(() => {
  ...
  function renderRevenueMixChart(viewModel) {
    ...
    const updateCenter = item => {
      ...
      centerLabel.textContent = item?.name || "Lifetime";
      centerValue.textContent = money(item?.value ?? viewModel.total);
    };
    chart.setOption({
      animation: false,
      aria: { enabled: true, description: `Lifetime gross revenue split across ${viewModel.packageCount} revenue-generating assets.` },
      tooltip: { show: false },
      series: [{
  ...
  function render() {
    ...
    const dashboardSummary = `<div class="upa-dashboard-summary"><section class="upa-dashboard-mix-card"><div class="upa-section-title"><div><small>ASSET ALLOCATION</small><h2>Revenue mix</h2></div></div>${revenueMixData.items.length ? `<div class="upa-revenue-mix-layout"><div class="upa-revenue-mix-visual"><div id="upa-revenue-mix-chart" class="upa-revenue-mix-chart" role="img" aria-label="Lifetime gross revenue contribution by asset"></div><div class="upa-revenue-mix-center"><small id="upa-revenue-mix-label">Lifetime</small><strong id="upa-revenue-mix-value">${money(revenueMixData.total)}</strong></div></div><div class="upa-revenue-concentration"><small>REVENUE CONCENTRATION</small><dl><div><dt>Largest asset share</dt><dd>${revenueMixData.largest ? percent(revenueMixData.largest.value / revenueMixData.total * 100) : "—"}</dd><span>${revenueMixData.largest ? escapeHtml(revenueMixData.largest.name) : "No revenue yet"}</span></div><div><dt>Top 3 share</dt><dd>${percent(revenueMixData.topThreeShare)}</dd><span>Of lifetime gross revenue</span></div><div><dt>Revenue-generating assets</dt><dd>${number(revenueMixData.packageCount)}</dd><span>With recorded gross revenue</span></div></dl></div></div>` : '<div class="upa-revenue-mix-empty">No gross revenue is available yet.</div>'}</section><div class="upa-kpis"><article><div><small>Gross revenue</small><span class="upa-kpi-dot upa-violet"></span></div><strong>${money(revenueChartData.total)}</strong><span>${number(paidUnits)} paid units in the selected period</span>${revenueChange}</article><article><div><small>Average monthly revenue</small><span class="upa-kpi-dot upa-violet"></span></div><strong>${money(trailingRevenue.monthlyAverage)}</strong><span>${trailingRevenue.monthCount === 12 ? "Last 12 complete months" : trailingRevenue.monthCount ? `Based on ${number(trailingRevenue.monthCount)} available months` : "No complete month available"}</span>${averageRevenueChange}</article><article><div><small>Revenue growth</small><span class="upa-kpi-dot ${revenueGrowthDot}"></span></div><strong>${revenueGrowthValue}</strong><span>${trailingRevenue.hasBaseline ? "Last 12 months versus previous 12" : "Needs 24 months of history"}</span></article><article><div><small>Pageviews</small><span class="upa-kpi-dot upa-cyan"></span></div><strong>${number(pageViews)}</strong><span>${number(salesQty)} purchases and claims</span>${pageViewsChange}</article><article><div><small>Conversion rate</small><span class="upa-kpi-dot upa-green"></span></div><strong>${percent(conversionRate)}</strong><span>Views that became purchases or claims</span>${conversionChange}</article><article><div><small>Downloads</small><span class="upa-kpi-dot upa-amber"></span></div><strong>${number(downloads)}</strong><span>Across the selected period</span>${downloadsChange}</article></div></div>`;
