(() => {
  ...
  function render() {
    ...
    const dashboardPackageTable = `<article class="upa-dashboard-packages"><div class="upa-section-title"><div><small>PACKAGE BREAKDOWN</small><h2>Package performance</h2><p>Selected-range results ranked by gross revenue, with trailing revenue context.</p></div><div class="upa-section-tools"><span>${number(packages.length)} packages</span><button class="upa-table-link" type="button" data-view="packages">Explore packages</button></div></div>${dashboardPackages.length ? `<div class="upa-package-table-wrap"><table class="upa-package-table"><thead><tr><th scope="col">Package</th><th scope="col">Gross revenue</th><th scope="col">Monthly average</th><th scope="col">12m growth</th><th scope="col">Conversion</th><th scope="col">Pageviews</th><th scope="col">Downloads</th></tr></thead><tbody>${dashboardPackages.map(item => `<tr><th scope="row"><div class="upa-package-identity"><span class="upa-package-avatar" aria-hidden="true">${escapeHtml(item.name?.trim().slice(0, 1).toUpperCase() || "P")}</span><span><strong>${escapeHtml(item.name)}</strong><small>${number(item.paidQty)} paid${item.freeQty ? ` · ${number(item.freeQty)} claims` : ""}</small></span></div></th><td><strong>${money(item.sales)}</strong><span class="upa-share-bar"><i style="width:${Math.min(100, item.share).toFixed(2)}%"></i></span><small>${percent(item.share)} of package revenue</small></td><td><strong>${item.trailing.monthCount ? money(item.trailing.monthlyAverage) : "—"}</strong><small>${item.trailing.monthCount === 12 ? "Last 12 complete months" : item.trailing.monthCount ? `${number(item.trailing.monthCount)} months available` : "No complete month"}</small></td><td><strong class="${revenueGrowthClass(item.trailing)}">${revenueGrowthLabel(item.trailing)}</strong><small>${item.trailing.hasBaseline ? "Versus previous 12m" : "Needs more history"}</small></td><td><strong>${item.pageViews ? percent(item.conversion) : "—"}</strong><small>${number(item.salesQty)} purchases and claims</small></td><td><strong>${number(item.pageViews)}</strong><small>Selected range</small></td><td><strong>${number(item.downloads)}</strong><small>Selected range</small></td></tr>`).join("")}</tbody></table></div><div class="upa-package-table-footer"><span>${packages.length > dashboardPackages.length ? `Showing the top ${dashboardPackages.length} of ${packages.length} packages` : `Showing all ${packages.length} packages in this range`}</span></div>` : '<div class="upa-package-table-empty">No package activity is available for this date range.</div>'}</article>`;
