(() => {
  ...
  let records = [];
  let prefs = { section: "dashboard", view: "revenue", range: "all", interval: "auto", start: "", end: "", calendarMetric: "sales", lifetimeStyle: "lines", lifetimeAlign: "calendar", lifetimePackages: [], lifetimeHiddenPackages: [], sankeyPackages: [], sankeyGroupBy: "category", sankeyCategoryDefaultApplied: true };
  let syncJob = null;
  ...
  function lifetimeViewModel(items, selectedPackageKeys, hiddenPackageKeys, requestedStyle, requestedAlign) {
    const packages = new Map();
    for (const item of items) {
    ...
    return { options, explicitKeys, activePackages, legend, series, total, leader, pointCount, align, style };
  }

  function renderLifetimeChart(viewModel) {
    const container = document.getElementById("upa-lifetime-chart");
    if (!container) return;
    if (!viewModel.series.length) { container.innerHTML = `<div class="upa-empty-chart">${viewModel.legend.length ? "Choose at least one package from the legend." : "No paid package revenue is available yet."}</div>`; return; }
