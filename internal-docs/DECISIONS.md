...
- **Current behavior:** One Performance Asset group menu contains the built-in "All assets" group, saved groups, individual assets, and one visually separated Manage groups action. The action opens dedicated pages where publishers can create, rename, change membership, and delete groups. Groups are stored under a publisher-scoped `chrome.storage.local` key and contain a generated group ID, a unique display name, and a deduplicated list of Unity `package_id` strings. "All assets" is built in and is not stored or editable.
